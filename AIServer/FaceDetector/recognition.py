...
class RecognitionHandler():
    ...
    def analysis(
        ...
        while True:
            ...
            faces_coordinates = []
            if freeze is False:
                faces_coordinates = self.grab_facial_areas(
                    img=img, detector_backend=detector_backend, anti_spoofing=anti_spoofing
                )

                detected_faces = self.extract_facial_areas(img=img, faces_coordinates=faces_coordinates)
                img = self.highlight_facial_areas(img=img, faces_coordinates=faces_coordinates)
                img = self.countdown_to_freeze(
                    img=img,
                ...
                if freeze:
                    ...
                    self.signin_signal.emit(target_name)
            ...
            freezed_img = self.countdown_to_release(img=freezed_img, tic=tic, time_threshold=time_threshold)
    ...
    def build_facial_recognition_model(self, model_name: str) -> None:
    ...
    def perform_facial_recognition(
        ...
        for idx, (x, y, w, h, is_real, antispoof_score) in enumerate(faces_coordinates):
            ...
            target_label, target_img, target_name= self.search_identity(
            ...
            img = self.overlay_identified_face(
                img=img,
