...
class DataProcessorThread(threading.Thread):
    ...
    def run(self):
        while self._is_running:
            try:
                ...
                elif parsed_data["camera_id"] == "Purchased":
                    self.process_purchase_cam(parsed_data["data"])
    ...
    def process_purchase_cam(self, data):
        """
        첫번째 request = [{"member_id": 1, "request_id": 1, "action": "get_cart"}]
        두번째 request = [{"member_id": 1, "request_id": 2, "action": "yes"}]
        """
        data = data[0]
        ...
        if visitor:
            ...
            if request_id == 1 and visitor.cart.purchase == 0:
                ...
                for fruit_name, price, quantity in cursor.fetchall():
                    res.append({"fruit_name": fruit_name, "price": price, "quantity": quantity})
                cursor.close()
                conn.close()
            ...
            # 두번째 request의 경우
            elif request_id == 2 and visitor.cart.purchase == 1:
                if action == "yes":
                    conn = self.thread_manager.connect_f2mbase()
                    cursor = conn.cursor()
        ...
        # 해당 member_id를 가진 visitor 객체가 없을 경우
        else:
            logger.info("No visitor info")
    ...
    def process_face_cam(self, data):
        ...
        if not visitor:
            ...
            # 에러
            logger.info("들어오지 않은 사용자가 계산하려고함")
